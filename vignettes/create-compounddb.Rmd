...
database. The function returns by default a `Spectra` object, that allows
to store multiple `Spectrum2` objects in a `list`-like structure with additional
annotations available in *metadata columns* (that can be accessed with
`mcols`).
...
In the example below we read compound and spectrum data from a
small SDF file that contains the first 7 spectra from an MoNa SDF file.
