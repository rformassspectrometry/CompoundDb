...
Before we create the `CompDb` database we load also MS/MS spectrum data from
some xml files from HMDB. For this we define the path to a directory containing
the respective xml files and use the `msms_spectra_hmdb` to import the spectrum
data into a single `data.frame`. Currently only the MS/MS spectrum xml files
from HMDB in the provided folder are processed.
...
At present only MS/MS spectra from xml files downloaded from HMDB are supported.
...
Analogous we can use the `spectra` function to extract spectrum data from the
database.
